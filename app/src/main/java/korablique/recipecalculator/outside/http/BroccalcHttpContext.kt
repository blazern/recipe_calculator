...
class BroccalcHttpContext @Inject constructor(
    ...
    suspend fun <T:Any> httpRequestAndUnwrap(
            url: String,
            resultType: KClass<T>,
    ...
    suspend fun <T:Any> httpRequest(
        ...
        val generalizedResponse = try {
            ...
            return BroccalcNetJobResult.Error.ResponseFormatError(
                    Exception("Response is neither $resultType or ${ServerErrorResponse::class}."
        ...
        if (generalizedResponse.status != STATUS_OK) {
            ...
            when (servErr.status) {
                STATUS_INVALID_CLIENT_TOKEN -> {
                    return BroccalcNetJobResult.Error.ServerError.NotLoggedIn(servErr)
                }
                STATUS_USER_NOT_FOUND -> {
                    return BroccalcNetJobResult.Error.ServerError.NotLoggedIn(servErr)
                }
                else -> {
                    return BroccalcNetJobResult.Error.ServerError.Other(servErr)
                }
            }
        }
        ...
        val result = try {
            ...
            return BroccalcNetJobResult.Error.ResponseFormatError(
                    Exception("Couldn't transform response into $resultType."
                            + "Response str: $responseStr", e))
        }
    ...
    private fun <T:Any> responseStrToType(
        ...
        if (typedResponse == null) {
            throw NullPointerException("Reponse was parsed as null."
